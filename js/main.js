...
const imgLines = document.querySelectorAll('.img-line');
...
function setActive(id) {
  ...
  imgLines.forEach(l => l.classList.toggle('active', l.dataset.id === id));
  detailTags.forEach(t => {
    const badge = t.querySelector('.detail-badge');
    t.classList.toggle('active', badge && badge.dataset.id === id);
  });
}
...
detailTags.forEach(t => {
  t.addEventListener('mouseenter', () => {
    const badge = t.querySelector('.detail-badge');
    if (badge) setActive(badge.dataset.id);
  });
...
// ========== Mobile hotspots: tap to toggle name ==========
const mHotspots = document.querySelectorAll('.m-hotspot');
mHotspots.forEach(h => {
  h.addEventListener('click', (e) => {
    e.stopPropagation();
    const isActive = h.classList.contains('active');
    mHotspots.forEach(o => o.classList.remove('active'));
    if (!isActive) h.classList.add('active');
  });
});
